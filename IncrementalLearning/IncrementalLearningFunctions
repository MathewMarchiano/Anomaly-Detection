...
class IncrementalLearning:
    ...
    def generateCodeword_Averaged(self, trainedClassifiers, holdoutData):
        '''
        ...
        # Get list of codewords using the holdout data.
        trainer = Trainer()
        listOfCodewords = trainer.getPredictions(holdoutData, trainedClassifiers)
    ...
    def generateCodeword_Mode(self, trainedClassifiers, holdoutData):
        '''
        Given a list of data and labels, this method will generate a codeword that (hopefully) doesn't already
        exist within the given codebook. This is intended to be used with only holdout classes for this reason.
